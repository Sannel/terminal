...
namespace
{
    // Scalar replacement for Microsoft::Console::Utils::FindActionableControlCharacter.
    // Returns pointer to first char32_t that is a C0 (<=0x1F), DEL (0x7F), or C1 (0x80-0x9F) control character.
    const char32_t* FindActionableControlCharacter(const char32_t* beg, const size_t len) noexcept
    {
...
static constexpr bool _isNumericParamValue(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isC0Code(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isC1ControlCharacter(const char32_t wch) noexcept
{
...
static constexpr char32_t _c1To7Bit(const char32_t wch) noexcept
{
    return wch - L'\x40';
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isIntermediate(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isDelete(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isEscape(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isParameterDelimiter(const char32_t wch) noexcept
{
    return wch == L';'; // 0x3B
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isSubParameterDelimiter(const char32_t wch) noexcept
{
    return wch == L':'; // 0x3A
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isCsiIndicator(const char32_t wch) noexcept
{
    return wch == L'['; // 0x5B
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isCsiPrivateMarker(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isIntermediateInvalid(const char32_t wch) noexcept
{
    // 0x30 - 0x3F
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isParameterInvalid(const char32_t wch) noexcept
{
    // 0x3C - 0x3F
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isStringTerminatorIndicator(const char32_t wch) noexcept
{
    return wch == L'\\'; // 0x5c
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isSs3Indicator(const char32_t wch) noexcept
{
    return wch == L'O'; // 0x4F
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isVt52CursorAddress(const char32_t wch) noexcept
{
    return wch == L'Y'; // 0x59
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isOscIndicator(const char32_t wch) noexcept
{
    return wch == L']'; // 0x5D
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isOscDelimiter(const char32_t wch) noexcept
{
    return wch == L';'; // 0x3B
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isOscInvalid(const char32_t wch) noexcept
{
    return wch <= L'\x17' ||
           wch == L'\x19' ||
           (wch >= L'\x1c' && wch <= L'\x1f');
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isOscTerminator(const char32_t wch) noexcept
{
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isDcsIndicator(const char32_t wch) noexcept
{
    return wch == L'P'; // 0x50
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isDcsPassThroughValid(const char32_t wch) noexcept
{
    // 0x20 - 0x7E
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isSosIndicator(const char32_t wch) noexcept
{
    return wch == L'X'; // 0x58
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isPmIndicator(const char32_t wch) noexcept
{
    return wch == L'^'; // 0x5E
}
...
// Return Value:
// - True if it is. False if it isn't.
static constexpr bool _isApcIndicator(const char32_t wch) noexcept
{
    return wch == L'_'; // 0x5F
}
...
// Return Value:
// - <none>
void StateMachine::_ActionExecute(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionExecuteFromEscape(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionPrint(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionPrintString(const std::u32string_view string)
{
    _SafeExecute([=, this]() {
...
// Return Value:
// - <none>
void StateMachine::_ActionEscDispatch(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionVt52EscDispatch(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionCsiDispatch(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionCollect(const char32_t wch) noexcept
{
...
// Return Value:
// - <none>
void StateMachine::_ActionParam(const char32_t wch)
{
    ...
    if (!_parameterLimitOverflowed)
    {
        ...
        if (_parameters.empty())
        {
            ...
            const auto rangeStart = static_cast<uint8_t>(_subParameters.size());
...
// Return Value:
// - <none>
void StateMachine::_ActionSubParam(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionOscParam(const char32_t wch) noexcept
{
...
// Return Value:
// - <none>
void StateMachine::_ActionOscPut(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionSs3Dispatch(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_ActionDcsDispatch(const char32_t wch)
{
    

    const auto success = _SafeExecute([=, this]() {
...
// Return Value:
// - <none>
void StateMachine::_EventGround(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventEscape(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventEscapeIntermediate(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventCsiEntry(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventCsiIntermediate(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventCsiIgnore(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventCsiParam(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventCsiSubParam(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventOscParam(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventOscString(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventOscTermination(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventSs3Entry(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventSs3Param(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventVt52Param(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventDcsEntry(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventDcsIntermediate(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventDcsParam(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventDcsPassThrough(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::_EventSosPmApcString(const char32_t /*wch*/) noexcept
{
...
// Return Value:
// - <none>
void StateMachine::ProcessCharacter(const char32_t wch)
{
...
// Return Value:
// - <none>
void StateMachine::ProcessString(const std::u32string_view string)
{
    size_t i = 0;
...
#pragma warning(suppress : 26481) // Don't use pointer arithmetic. Use span instead (bounds.1).)
            ...
            const auto it = FindActionableControlCharacter(beg, len);
    ...
    if (_state != VTStates::Ground)
    {
        ...
        if (cacheUnusedRun)
        {
            if (!_cachedSequence)
            {
                _cachedSequence.emplace(std::u32string{});
            }
...
void StateMachine::_AccumulateTo(const char32_t wch, VTInt& value) noexcept
{
