...
namespace KTerm
{
    KTerminal::KTerminal(int rows, int cols, QObject* parent) :
        QObject(parent),
        _buffer(std::make_unique<TextBuffer>(rows, cols))
    {
        ...
        auto dispatch = std::make_unique<KTerminalDispatch>(*_buffer);
    ...
    void KTerminal::Resize(int rows, int cols)
    {
    ...
    void KTerminal::_onPtyOutput(std::string_view data)
    {
    ...
    void KTerminal::_decodeUtf8(unsigned char byte)
    {
        if (_utf8Remaining > 0)
        {
            // Continuation byte.
